...
function EncodingParametersInURLs() {
  ...
  const fetchMultiply = async (a: number, b: number) => {
    const response = await axios.get(
      `${API_BASE}/a5/multiply/${a}/${b}`);
    setResult(response.data);
  };

  const fetchDivide = async (a: number, b: number) => {
    const response = await axios.get(
  ...
  return (
    <div>
      ...
      <button onClick={() => fetchMultiply(a, b)} >
        Fetch Multiply of {a} * {b}
      </button>
      <button onClick={() => fetchDivide(a, b)} >
      ...
      <a className="btn btn-primary" href={`${API_BASE}/a5/subtract/${a}/${b}`}>
        Substract {a} - {b}
      </a>
